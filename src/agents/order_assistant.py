...
class OrderState(MessagesState, total=False):
    """State management for order processing"""
...
# Example tools - you would implement these based on your actual database/systems
...
async def acall_model(state: OrderState, config: RunnableConfig) -> OrderState:
    ...
    # Ensure we have enough steps for the complete flow
...
async def llama_guard_input(state: OrderState, config: RunnableConfig) -> OrderState:
    ...
    # Return partial state update preserving messages
    return {**state, "safety": safety_output}


async def block_unsafe_content(state: OrderState, config: RunnableConfig) -> OrderState:
    safety_output = state.get("safety")
    if safety_output is None:
        # Fallback if safety is not set
        return {"messages": [AIMessage(content="Content safety check failed.")]}
    return {"messages": [format_safety_message(safety_output)]}
...
def check_safety(state: OrderState) -> Literal["unsafe", "safe"]:
    safety_output = state.get("safety")
    if safety_output is None:
        return "safe"  # Default to safe if no safety assessment
    match safety_output.safety_assessment:
...
order_assistant = agent.compile(checkpointer=MemorySaver())
