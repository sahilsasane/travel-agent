...
async def main() -> None:
    ...
    if len(messages) == 0:
        match agent_client.agent:
            ...
            case "travel-planner":
                WELCOME = """Hello! I'm an AI-powered travel planner. I can help you find flights, book them, and ask you for more information if needed."""
            case _:
    ...
    try:
        while msg := await anext(messages_agen, None):
            ...
            match msg.type:
                        ...
                        if msg.tool_calls:
                            ...
                            for _ in range(len(call_results)):
                                try:
                                    ...
                                    # Handle case where we get a string instead of ChatMessage
                                    if isinstance(tool_result, str):
                                        # Skip streaming tokens during tool execution
                                        continue

                                    if not isinstance(tool_result, ChatMessage):
                                        st.error(
                                            f"Unexpected tool result type: {type(tool_result)}"
                                        )
                                    ...
                                    if (
                                        ...
                                        and tool_result.tool_call_id in call_results
                                    ):
                                        status = call_results[tool_result.tool_call_id]
                                    else:
                                        # Fallback to the first status if tool_call_id doesn't match
                                        status = next(iter(call_results.values()))
                                    status.write("Output:")
                ...
                case "tool":
                    # Tool messages should be handled within the AI message case above
                    # But if we get one independently, handle it gracefully
                    if is_new:
                        st.session_state.messages.append(msg)
